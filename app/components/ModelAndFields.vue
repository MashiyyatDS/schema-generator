		...
		<div class="grid grid-cols-12 gap-1">
			<div class="col-span-8">
				<UFormField label="Model Name">
					<UInput v-model="modelFields.name" placeholder="Enter your model name" class="w-full" size="lg" />
				</UFormField>
			</div>
			...
			<div class="col-span-4">
				<UFormField label="Model field name">
					<div class="flex gap-1">
						<UInput v-model="fieldName" placeholder="Enter your model name" class="w-full" size="lg" @keyup.enter="addField" />
			...
			<div v-for="(field, key) in modelFields.fields" :key="key" class="col-span-12 flex gap-1">
				<UButton
    ...
    public function scopeSearch(Builder $query, ?string $search): void
    {
        $query->where('id', $search)
			${modelFields.value.fields
				.map((field) => `->orWhere('${convertStringCases(field).snake}', 'like', "%{$search}%")\n\t\t\t`)
