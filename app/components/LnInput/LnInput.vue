<template>
	<div v-if="!lnInput.hidden" :class="[lnInput.grid ?? 'col-span-12']">
		<UFormField :label="lnInput.label" :description="lnInput.description" :error="lnInput.errors" :hint="lnInput.hint" :help="lnInput.help">
			...
			<UPinInput v-if="lnInput.type === 'pin'" v-model="lnInput.value" v-bind="lnInput.attributes" @update:model-value="onValueUpdated" />
			...
			<USwitch v-if="lnInput.type === 'switch'" v-model="lnInput.value" v-bind="lnInput.attributes" @update:model-value="onValueUpdated" />

			<USlider v-if="lnInput.type === 'slider'" v-model="lnInput.value" v-bind="lnInput.attributes" @update:model-value="onValueUpdated" />
